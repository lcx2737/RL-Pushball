...
#                               [world.landmarks[0].state.p_pos - world.landmarks[1].state.p_pos] + [world.landmarks[1].state.p_pos - agent.state.p_pos])
        return  np.concatenate([agent.state.p_vel] + [agent.state.p_pos] + other_pos + 
                               [world.landmarks[0].state.p_pos] + [world.landmarks[1].state.p_pos])

    def done(self, agent, world):
        dis_2landmark = np.sqrt(np.sum(np.square(world.landmarks[1].state.p_pos - world.landmarks[0].state.p_pos)))
        return True if dis_2landmark <= (world.landmarks[1].size + world.landmarks[0].size) else False
